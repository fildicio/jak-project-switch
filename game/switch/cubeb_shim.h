...
static inline void sdl_shim_audio_callback(void* userdata, Uint8* stream, int len) {
  // FIX 7m: NO LOGGING HERE. This runs on SDL's audout thread with a hard realtime deadline,
  // and switch_run_logf does an fsync'd SD write under the global filesystem lock -- putting
  // the audio thread behind the ISO thread's streaming reads. The 7j/7k probes proved what
  // they needed to (the callback runs and returns normally), so they are gone.
...
static inline int cubeb_init(cubeb** context, const char* /*name*/, const char* /*backend*/) {
  switch_run_logf("[aud] cubeb_init: SDL_InitSubSystem(AUDIO)");
  if (SDL_InitSubSystem(SDL_INIT_AUDIO) != 0) {
    switch_run_logf("[aud] cubeb_init FAILED: %s", SDL_GetError());
    return CUBEB_ERROR;
  }
  switch_run_logf("[aud] cubeb_init ok, driver=%s",
...
static inline int cubeb_stream_init(cubeb* /*context*/,
  ...
  switch_run_logf("[aud] SDL_OpenAudioDevice want: freq=%d ch=%u samples=%u", want.freq,
                  (unsigned)want.channels, (unsigned)want.samples);
  s->device = SDL_OpenAudioDevice(nullptr, 0, &want, &have, 0);
  if (s->device == 0) {
    switch_run_logf("[aud] SDL_OpenAudioDevice FAILED: %s", SDL_GetError());
    delete s;
    return CUBEB_ERROR;
  }
  switch_run_logf("[aud] SDL_OpenAudioDevice ok: dev=%u have freq=%d ch=%u samples=%u size=%u",
                  (unsigned)s->device, have.freq, (unsigned)have.channels,
                  (unsigned)have.samples, (unsigned)have.size);
...
static inline int cubeb_stream_start(cubeb_stream* stream) {
  switch_run_logf("[aud] cubeb_stream_start dev=%u", stream ? (unsigned)stream->device : 0u);
  if (!stream || stream->device == 0) {
    return CUBEB_ERROR;
  }
  SDL_PauseAudioDevice(stream->device, 0);
  switch_run_logf("[aud] cubeb_stream_start unpaused");
  if (stream->state_cb) {
