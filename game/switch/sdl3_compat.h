...
// Set from an atexit handler (see InputManager's constructor). Once the process has
// started exiting, libnx's __appExit() tears down the HID shared memory -- but SDL's
// Switch video driver keeps polling the *touchscreen* from PumpEvents, which quitting
// SDL_INIT_GAMEPAD does not stop. Any hidGetTouchScreenStates() after that aborts the
// process with a misleading 0x1159 "User Break" report that hides whatever actually
// went wrong (e.g. a frozen title being closed from the HOME menu). Once set, event
// translation stops calling into SDL entirely so nothing touches HID anymore.
inline std::atomic<bool> sdl3compat_shutdown{false};

inline int sdl3compat_PollEvent(SDL_Event* out) {
  if (sdl3compat_shutdown.load(std::memory_order_relaxed)) {
    return 0;
  }
